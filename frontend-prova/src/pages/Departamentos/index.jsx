...
export default function Departamentos() {
  ...
  async function handleFiltrar(pageIndex = 0) {
    ...
    try {
      ...
      console.error("Erro ao filtrar departamentos:", error);
    } finally {
      setCarregando(false);
    }
  }
  ...
  async function handleBaixarRelatorio() {
    try {
      await baixarRelatorioPdf(
        '/departamentos/relatorio',
        {
          descricao: filtroDescricao,
          codigo: filtroCodigo,
        },
        'relatorio-departamentos.pdf'
      );
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-4xl font-bold text-[#3078b4] mb-1">Departamentos</h1>
          <p className="text-gray-500 text-sm">Veja os departamentos cadastrados no sistema.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#eef2f6]">
              <tr>
                ...
                <th className="py-3 px-4 text-[#3078b4] font-bold text-sm">Descrição</th>
            ...
            <tbody>
              {carregando ? (
                ...
                departamentos.map((depto) => (
                  <tr key={depto.id} className="border-t border-gray-200 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4 text-center">
                      <button
                        onClick={() => navigate(`/departamentos/editar/${depto.id}`)}
                    ...
                    <td className="py-3 px-4 text-sm font-bold text-black">{depto.descricao}</td>
                    <td className="py-3 px-4 text-sm font-bold text-black">{depto.codigoDepartamento || depto.codigo}</td>
                  </tr>
                ))
              )}
            </tbody>
